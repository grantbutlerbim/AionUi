...
import type TaskAgentManager from '../task/TaskAgentManager';
...
export function initConversationBridge(): void {
  ...
  ipcBridge.conversation.stop.provider(async ({ conversation_id }) => {
    ...
    if (task.type !== 'gemini' && task.type !== 'acp' && task.type !== 'codex' && task.type !== 'openclaw-gateway' && task.type !== 'task') {
  ...
  ipcBridge.conversation.sendMessage.provider(async ({ conversation_id, files, ...other }) => {
    ...
    let task: GeminiAgentManager | AcpAgentManager | CodexAgentManager | OpenClawAgentManager | TaskAgentManager | undefined;
    try {
      task = (await WorkerManage.getTaskByIdRollbackBuild(conversation_id)) as GeminiAgentManager | AcpAgentManager | CodexAgentManager | OpenClawAgentManager | TaskAgentManager | undefined;
    } catch (err) {
    ...
    try {
      ...
      if (task.type === 'gemini') {
        ...
        await (task as TaskAgentManager).sendMessage({ content: other.input, files: workspaceFiles, msg_id: other.msg_id });
        return { success: true };
      } else {
